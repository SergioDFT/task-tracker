...
function TaskDetailClient({ task }: { task: any }) {
...
// This is our actual page component that will run on the server
export default async function TaskDetailPage({ params }: { params: { id: string } }) {
  try {
    // Fetch the task data server-side
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/tasks/${params.id}`, {
      cache: "no-store",
    });
    
    if (!response.ok) {
      // If task not found, show 404 page
    ...
    // Render the task detail client component with the fetched data
    return (
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        <div className="bg-red-50 text-red-800 p-4 rounded-md">
          ...
          <div className="mt-4">
            <Link 
              href="/dashboard"
              className="inline-flex items-center text-red-800 hover:text-red-900"
            >
              <ArrowLeftIcon className="h-4 w-4 mr-1" />
              Return to Dashboard
            </Link>
